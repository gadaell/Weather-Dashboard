...
function showFiveDayForecast() {
  fiveDayForecastEl.removeAttribute("class", "hide");
}
...
function getNextFiveDays() {
  ...
  document.getElementById("cardDate0").innerHTML = " " + nextFiveDayDates[1];
  document.getElementById("cardDate1").innerHTML = " " + nextFiveDayDates[2];
  document.getElementById("cardDate2").innerHTML = " " + nextFiveDayDates[3];
  document.getElementById("cardDate3").innerHTML = " " + nextFiveDayDates[4];
  document.getElementById("cardDate4").innerHTML = " " + nextFiveDayDates[5];
...
function searchCurrentCity(city) {
  ...
  fetch(
    `https://api.openweathermap.org/geo/1.0/direct?q=${city}&appid=cfc3aec1f6ff3281aaccd22614582b10`
  )
    ...
    .then(function (data) {
      ...
      document.getElementById("currentCityName").innerHTML = " " + data[0].name;

      document.getElementById("currentCityState").innerHTML =
        " " + data[0].state;
      ...
      fetch(
        `https://api.openweathermap.org/data/2.5/weather?lat=${lat}&lon=${lon}&units=imperial&appid=cfc3aec1f6ff3281aaccd22614582b10`
      )
        .then(function (response) {
          return response.json();
        })
        .then(function (data) {
          ...
          var currentDayIconEl = document.getElementById("currentCityIcon");
      ...
      //get UV index
      fetch(`https://api.openweathermap.org/data/2.5/onecall?lat=${lat}&lon=${lon}&units=imperial&appid=cfc3aec1f6ff3281aaccd22614582b10
        `)
        .then(function (response) {
          return response.json();
        })
        .then(function (data) {
          document.getElementById("currentCityUvindex").innerHTML =
      ...
      fetch(`https://api.openweathermap.org/data/2.5/forecast?lat=${lat}&lon=${lon}&units=imperial&appid=cfc3aec1f6ff3281aaccd22614582b10
        `)
        .then(function (response) {
          return response.json();
        })
...
function fiveDayForecast(forecastResponse) {
  ...
  for (var i = 0; i < foreCastItemsArray.length; i++) {
    document.getElementById("cardtemp" + i).innerHTML =
      "Temp: " + foreCastItemsArray[i].temp + " °F";
    document.getElementById("cardwind" + i).innerHTML =
      "Wind: " + foreCastItemsArray[i].wind + " mph.";
    document.getElementById("cardhumidity" + i).innerHTML =
    ...
    // Step 3: created Icon HTML element variable via HTML Id.
    var fiveDayIconEl = document.getElementById("cardicon" + i);
...
function getSavedCities() {
  ...
  for (var i = 0; i < savedCityNames.length; i++) {
    ...
    var buttonDiv = document.getElementById("searchedCityButtons");
